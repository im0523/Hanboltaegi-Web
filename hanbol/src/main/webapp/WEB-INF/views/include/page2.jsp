...
<c:if test="${cpage.curBlock gt 1}">
	<a onclick="go_page(1)" class="page-first">처음</a>
	<a onclick="go_page(${cpage.beginPage-cpage.blockPage})" class="page-prev">이전</a>
</c:if>

<c:forEach var="no" begin="${cpage.beginPage}"
					end="${cpage.endPage }">
	<c:if test="${no eq cpage.curPage}">
		<span class="page-on">${no}</span>
	</c:if>
	<c:if test="${no ne cpage.curPage}">
		<a onclick="go_page(${no})" class="page-off">${no}</a>
	</c:if>	
</c:forEach>

<c:if test="${cpage.curBlock lt cpage.totalBlock}">
	<a onclick="go_page(${cpage.endPage+1})" class="page-next">다음</a>
	<a onclick="go_page(${cpage.totalPage})" class="page-last">마지막</a>
</c:if>
